...
def monotransit_detection(pts, conf=None, peak_thresh=0.8, agg_fn=np.multiply, score_fn=np.max, smooth=True):
    ...
    tr_indc = rnndet.get_peaks(pts_>0.8)
    
    conf_scores = [score_fn(agg_fn(conf_[indc], pts_[indc])) for indc in tr_indc]
